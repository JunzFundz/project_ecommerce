        ...
        $.ajax({
            ...
            success: function (response) {
                $.each(response, function (key, value) {
                    ...
                    // Decode and display the images from the JSON array
                    const images = JSON.parse(value['img']);

                    // Clear previous images from the preview
                    $('#imagePreview').html('');

                    // Loop through the images and create img elements with delete button (X)
                    $.each(images, function (index, image) {
                        ...
                        // Append each image (with delete button) to the preview container
                ...
                // Add event listener for delete button (X)
                $('#imagePreview').on('click', '.delete-image', function (e) {
                    e.preventDefault();

                    const imageName = $(this).data('image');  // Get the image name to delete
                    const imageElement = $(this).closest('.image-container');  // Get the container of the image

                    // Send AJAX request to delete image from the database
                    $.ajax({
                        url: 'database/additem.php',  // Endpoint to handle the image deletion
                        type: 'POST',
                        data: {
                            'imageName': imageName,  // Send the image name to be deleted
                        },
                        success: function (response) {
                            if (response.success) {
                                // Remove the image from the UI upon successful deletion
                                imageElement.remove();
                            } else {
